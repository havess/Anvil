...
void ImGui::ShowDemoWindow(bool *p_open) {
  ...
  if (ImGui::CollapsingHeader("Configuration")) {
    ...
    if (ImGui::TreeNode("Capture/Logging")) {
      ...
      HelpMarker("Try opening any of the contents below in this window and "
...
static void ShowDemoWindowWidgets() {
  ...
  if (ImGui::TreeNode("Trees")) {
    ...
    if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
      ...
      int node_clicked = -1; // Temporary storage of what node we have clicked
                             // to process selection at the end of the loop. May
                             // be a pointer to your own node type, etc.
  ...
  if (ImGui::TreeNode("Text Input")) {
    ...
    if (ImGui::TreeNode("Resize Callback")) {
      ...
      // std::string.
      HelpMarker("Demonstrate using ImGuiInputTextFlags_CallbackResize to wire "
  ...
  if (ImGui::TreeNode("Color/Picker Widgets")) {
    ...
    ImGui::SameLine();
    HelpMarker("Click on the colored square to open a color "
               "picker.\nCTRL+click on individual component to input value.\n");
    ...
    HelpMarker("SetColorEditOptions() is designed to allow you to set "
               ...
               "you can force options on a per-widget basis if needed, and the "
               "user can change non-forced ones with the options menu.\nWe "
               "don't have a getter to avoid encouraging you to persistently "
      ...
      for (int n = 0; n < IM_ARRAYSIZE(names); n++) {
        ...
        if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
          ImGui::SetDragDropPayload(
              "DND_DEMO_CELL", &n,
              sizeof(int)); // Set payload to carry the index of our item (could
                            // be anything)
          if (mode == Mode_Copy) {
...
static void ShowDemoWindowLayout() {
  ...
  if (ImGui::TreeNode("Widgets Width")) {
    ...
    ImGui::Text(
        "SetNextItemWidth/PushItemWidth(GetContentRegionAvailWidth() * 0.5f)");
    ImGui::SameLine();
  ...
  if (ImGui::TreeNode("Text Baseline Alignment")) {
    HelpMarker("This is testing the vertical alignment that gets applied on "
               "text to keep it aligned with widgets. Lines only composed of "
               "text or \"small\" widgets fit in less vertical spaces than "
  ...
  if (ImGui::TreeNode("Horizontal Scrolling")) {
    HelpMarker("Horizontal scrolling for a window has to be enabled explicitly "
...
static void ShowDemoWindowColumns() {
  ...
  */
  ...
  ImGui::SameLine();
  HelpMarker("NB: Tree node must be poped before ending the cell. There's no "
    ...
    if (ImGui::BeginTabItem("Fonts")) {
      ...
      for (int i = 0; i < atlas->Fonts.Size; i++) {
        ...
        if (font_details_opened) {
          ...
          ImGui::SameLine();
          HelpMarker("Note than the default embedded font is NOT meant to be "
                     "scaled.\n\nFont are currently rendered into bitmaps at a "
                     "given size at the time of building the atlas. You may "
                     "oversample them to get some flexibility with scaling. "
                     "You can also render at multiple sizes and select which "
                     "one to use at runtime.\n\n(Glimmer of hope: the atlas "
                     "system should hopefully be rewritten in the future to "
...
struct MyDocument {
  ...
  bool Open; // Set when the document is open (in this demo, we keep an array of
             // all available documents to simplify the demo)
